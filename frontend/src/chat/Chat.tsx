...
export default function Chat({ conversationId }: { conversationId: string }) {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:8000/api/conversations/${conversationId}/messages/`)
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      alert(err.response?.data?.detail || "Erreur lors de l'envoi du message");
    }
  };

  // Identifier "moi" pour aligner à droite
  const myId = localStorage.getItem("user_id"); // ou récupère depuis redux/auth
